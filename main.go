...
func showMenu(filename string, metadata string, size int64) {
	...
	fmt.Printf("%s      ::::::::  :::    ::: :::::::::  :::::::::: :::::::::  :::::::::  :::    ::: ::::    ::: :::    ::: %s\n", Cyan, Reset)
	fmt.Printf("%s    :+:    :+: :+:    :+: :+:    :+: :+:        :+:    :+: :+:    :+: :+:    :+: :+:+:   :+: :+:   :+:   %s\n", Cyan, Reset)
	fmt.Printf("%s ───────────────────────────────────────[ NATIVE GO FORGE ENGINE v7.0 ]───────────────────────────────────────%s\n", Gray, Reset)
	...
	fmt.Printf("   %s│%s  %s3)%s Precision String Placement Tool (Interactive Confirmation)\n", Gray, Reset, Green, Reset)
	...
	fmt.Printf("   %s│%s  %s8)%s Diagnostic Section Map & Structural Layout Exposer\n", Gray, Reset, Blue, Reset)
	fmt.Printf("   %s│%s  %s9)%s Abort Terminal Session\n", Gray, Reset, Red, Reset)
	fmt.Printf("%s ───────────────────────────────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
...
case 4:
	clearTerminal()
	fmt.Printf("%s>>> AUTOMATED MATRIX DECOMPRESSION SYSTEM (UPX CORE)%s\n\n", Orange, Reset)
	...
	// Validate if the target binary possesses the explicit raw UPX signature pattern
	if !bytes.Contains(data, []byte("UPX!")) {
		fmt.Printf(" %s[-] Target Scan Clean: Standard UPX! headers are absent from this binary layout.%s\n", Yellow, Reset)
		fmt.Printf("    If the file entropy calculation is spiked, it likely implements a custom cryptor.\n")
		fmt.Printf("\n%sPress [ENTER] to return to the dashboard menu...%s", Gray, Reset)
		_, _ = fmt.Scanln()
		continue
	}
			
	// Verify if the system has the upx unpack tool binary configured in the environment path
	...
	// Trigger a clean sub-process environment command invocation to execute decompression in-place
	...
	if execErr != nil {
		...
		// Re-read file stats to update the size metrics displayed on the HUD dashboard dynamically
...
case 5:
	...
	case 6:
		...
		case 7:
			...
			localOutputFile := targetFile + "_patched"
			// Check if the patched binary actually exists before attempting to sign it
			if _, err := os.Stat(localOutputFile); os.IsNotExist(err) {
				fmt.Printf(" %s❌ FAULT: Target patched file '%s' not found.%s\n", Red, localOutputFile, Reset)
				fmt.Printf("    Please run a patching module (Option 2, 3, or 6) first to generate the target file.\n")
				fmt.Printf("\n%sPress [ENTER] to return...%s", Gray, Reset)
				_, _ = fmt.Scanln()
				continue
			}
			...
			err = signCmd.Run()
			_ = os.Remove("extracted_ents.xml") // Clean up instantly from workspace
		...
		case 8:
			clearTerminal()
			fmt.Printf("%s>>> STRUCTURAL BINARY BLUEPRINT & SECTION MAP EXPOSER%s\n\n", Blue, Reset)
			runLoadingBar("PARSING_INTERNAL_HEADER_TABLES_AND_MAPPING_ALIGNMENTS")
			
			// A: Check macOS Mach-O File Format Internals
			if machoFile, err := macho.Open(targetFile); err == nil {
				defer machoFile.Close()
				fmt.Printf("%s[+] Mach-O Segment & Section Breakdown Matrix:%s\n", Green, Reset)
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				printfFormat := "  %-12s %-16s %-12s %-12s %-10s\n"
				fmt.Printf(printfFormat, "SEGMENT", "SECTION", "VIRT_ADDR", "RAW_OFFSET", "SIZE_BYTES")
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				for _, sec := range machoFile.Sections {
					fmt.Printf(printfFormat, sec.Seg, sec.Name, fmt.Sprintf("0x%X", sec.Addr), fmt.Sprintf("0x%X", sec.Offset), fmt.Sprintf("%d", sec.Size))
				}
				
			// B: Check Linux ELF File Format Internals
			} else if elfFile, err := elf.Open(targetFile); err == nil {
				defer elfFile.Close()
				fmt.Printf("%s[+] ELF Object Section Blueprint Layout:%s\n", Green, Reset)
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				printfFormat := "  %-24s %-16s %-16s %-12s\n"
				fmt.Printf(printfFormat, "SECTION_NAME", "VIRT_ADDR", "FILE_OFFSET", "SIZE_BYTES")
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				for _, sec := range elfFile.Sections {
					fmt.Printf(printfFormat, sec.Name, fmt.Sprintf("0x%X", sec.Addr), fmt.Sprintf("0x%X", sec.Offset), fmt.Sprintf("%d", sec.Size))
				}
				
			// C: Check Windows PE File Format Internals
			} else if peFile, err := pe.Open(targetFile); err == nil {
				defer peFile.Close()
				fmt.Printf("%s[+] PE Execution Section Header Matrix:%s\n", Green, Reset)
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				printfFormat := "  %-16s %-16s %-16s %-12s\n"
				fmt.Printf(printfFormat, "SECTION_NAME", "VIRT_ADDR", "RAW_OFFSET", "SIZE_BYTES")
				fmt.Printf("%s ──────────────────────────────────────────────────────────────────────────────%s\n", Gray, Reset)
				for _, sec := range peFile.Sections {
					fmt.Printf(printfFormat, sec.Name, fmt.Sprintf("0x%X", sec.VirtualAddress), fmt.Sprintf("0x%X", sec.Offset), fmt.Sprintf("%d", sec.Size))
				}
				
			// D: Fallback for generic data payloads
			} else {
				fmt.Printf(" %s[-] Telemetry Halt: Target is a raw data stream. Standard execution headers absent.%s\n", Yellow, Reset)
			}
